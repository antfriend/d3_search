d3.json('example_network.json', function(network_data) {
  ...
  // generate a list of concepts for auto complete
  ////////////////////////////////////////////////
  // get all concepts
  all_concepts = [];
  ...
  for (i = 0; i < row_nodes.length; i++) {
    all_concepts.push(row_nodes[i].name);
  }
...
function find_gene_in_clust() {
  ...
  if (all_concepts.indexOf(search_gene) != -1) {
...
function initialize_clustergram(network_data) {
  ...
  scale_reduce_font_size_factor = d3.scale.log().domain([min_node_num, max_node_num]).range([min_fs_zoom, max_fs_zoom])
    .clamp('true');
...
function set_visualization_size() {
  ...
  min_label_width = 60;
  max_label_width = 200;
  label_scale = d3.scale.linear().domain([min_num_char, max_num_char]).range([min_label_width, max_label_width]).clamp(
    'true');
...
function apply_transformation(trans_x, trans_y, zoom_x, zoom_y, duration) {
  ...
  clust_group
    .attr('transform', 'translate(' + [margin.left + trans_x, margin.top + trans_y] + ') scale(' + zoom_x + ',' +
      zoom_y + ')');
...
function two_translate_zoom(pan_dx, pan_dy, fin_zoom) {
  ...
  var shift_top_viz = initIfnew(shift_top_viz);
  var shift_up_viz = initIfnew(shift_up_viz);
  ...
  if (pan_dy >= half_height - y_pan_room) {
    ...
    shift_up_viz = -half_height / zoom_switch + shift_top_viz;
  ...
  if (pan_dy < -(half_height - y_pan_room)) {
    ...
    shift_up_viz = half_height / zoom_switch - shift_top_viz; //- move_up_one_row;
  ...
  clust_group
    ...
    .attr('transform', 'translate(' + [margin.left, margin.top + center_y] + ')' + ' scale(' + 1 + ',' + zoom_y + ')' +
  ...
  d3.select('#row_labels')
    .transition()
    .duration(search_duration)
    .attr('transform', 'translate(' + [row_margin.left, margin.top + center_y] + ')' + ' scale(' + zoom_y + ',' +
      zoom_y + ')' + 'translate(' + [0, pan_dy] + ')');
  ...
  d3.select('#col_labels')
    ...
    .attr('transform', 'translate(' + [col_margin.left, col_margin.top] + ')' + ' scale(' + 1 + ',' + 1 + ')' +
      'translate(' + [pan_dx, 0] + ')');
...
function zoom_and_highlight_found_gene(search_gene) {
  ...
  inst_gene_index = _.indexOf(all_concepts, search_gene);
